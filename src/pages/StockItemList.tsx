    ...
    <Layout>
      <div className="p-6 space-y-5">
        <div className="flex items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold">{locale === 'ko' ? '품목별 재고 목록' : 'Item Stock List'}</h1>
              <LanguageToggle />
            </div>
            <p className="text-sm text-slate-400 mt-1">
              {locale === 'ko'
                ? 'SKU 단위 통합 재고 조회 · 문서 경로 `/stock/items` 반영'
                : 'Integrated SKU-level stock view (`/stock/items`).'}
            </p>
          </div>
          <button
            onClick={() =>
        ...
        <div className="bg-[#1e293b] border border-slate-700/50 rounded-xl p-4 grid grid-cols-1 lg:grid-cols-5 gap-3">
          ...
          <input value={attrKeyword} onChange={(e) => { setAttrKeyword(e.target.value); setPage(1) }} placeholder={locale === 'ko' ? '품목속성(스타일/컬러/사이즈)' : 'Attributes (style/color/size)'} className="px-3 py-2.5 bg-slate-700 border border-slate-600 rounded-lg text-sm" />
          <button
            onClick={() => {
              setSkuKeyword('')
              setNameKeyword('')
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-700 text-slate-400">
                ...
                <th className="text-left px-4 py-3 font-medium">{locale === 'ko' ? '품목명/속성' : 'Item/Attributes'}</th>
                <th className="text-left px-4 py-3 font-medium">Zone</th>
                <th className="text-right px-4 py-3 font-medium">{locale === 'ko' ? '총재고' : 'Total qty'}</th>
                ...
                <th className="text-left px-4 py-3 font-medium">{locale === 'ko' ? '최종 이동일' : 'Last moved'}</th>
              </tr>
            </thead>
            <tbody>
              {paged.map((item) => (
                <tr key={item.sku} className="border-b border-slate-700/40 hover:bg-slate-700/30">
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-xs px-2 py-1 rounded-full ${statusBadge[item.status]}`}>
                      {locale === 'ko'
                        ? statusLabel[item.status]
                        : ({ normal: 'Available', low: 'Low stock', excess: 'Excess', defect: 'Defect/Quarantine' }[item.status] ?? item.status)}
                    </span>
                  </td>
